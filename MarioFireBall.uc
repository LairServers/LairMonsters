...
simulated function HitWall(vector HitNormal, actor Wall)
{
	...
	else Velocity = MirrorVectorByNormal(Velocity,HitNormal)*0.8f;
}
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
    ...
    {
        Spawn(class'FlameImpact',,,HitLocation + HitNormal*20,rotator(HitNormal));
...
simulated function HurtRadius( float DamageAmount, float DamageRadius, class<DamageType> DamageType, float Momentum, vector HitLocation )
{
    ...
    foreach CollidingActors (class 'Actor', Victims, DamageRadius, HitLocation)
    {
        ...
        if( (Victims != self) && (Victims != Instigator) &&(Hurtwall != Victims)
            ...
            && ExtendedZCollision(Victims)==None && KFBulletWhipAttachment(Victims)==None && ZEDMario(Victims)==None )
        {
...
{
	if ( KFBulletWhipAttachment(Other)!=None || ExtendedZCollision(Other)!=none || ZEDMario(Other)!=None )
		return;
	super.Touch(Other);
}
